...
{			
	...
	$.ajax
	({
		url: 'https://catalog.planoplan.com/api/v2/search/?keys[0]='+libs+'&disregard_price=1&disregard_structure=1&id[0]='+cdm[0].lotid+'&lang=ru',
		type: 'GET', 
		dataType: 'json',
		success: function(json)
		{ 			
			// если лота нету в массиве, то добавляем его как пустой объект
...
function loaderWD(cdm, json)
{
	...
	arrPop[arrPop.length] = cdm.emptyBox;
...
{			
	...
	{  
		$.ajax
		({
			url: 'https://catalog.planoplan.com/api/v2/search/?keys[0]='+libs+'&disregard_price=1&disregard_structure=1'+list+'&lang=ru',
			type: 'GET', 
			dataType: 'json',
			success: function(json)
			{ 				
